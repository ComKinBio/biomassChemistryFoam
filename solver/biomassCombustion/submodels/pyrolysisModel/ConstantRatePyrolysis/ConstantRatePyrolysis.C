...
#include "ConstantRatePyrolysis.H"

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

template<class CloudType>
Foam::ConstantRatePyrolysis<CloudType>::ConstantRatePyrolysis
(
    ...
    residualCoeff_(readScalar(this->coeffDict().lookup("residualCoeff")))
{
    ...
    {
        WarningInFunction
            << "Pyrolysis model selected, but no volatiles defined"
            << nl << endl;
    }
    else
    {
...
template<class CloudType>
void Foam::ConstantRatePyrolysis<CloudType>::calculate
(
    const scalar dt,
    const scalar age,
    const scalar mass0,
